rWhat <- function(raster,northing, easting)
{
    if (nchar(Sys.getenv("GISRC")) > 0) {
      oechoCmd <- get.echoCmdOption()       
      set.echoCmdOption(TRUE)
      rawOut <- execGRASS("r.what",map=raster,coordinates=c(easting,northing))
      rawOut <- attr(rawOut,"resOut")
      rawOut <- strsplit(rawOut,"[|]")
      rastValue <- as.numeric(tail(rawOut[[1]],1))
